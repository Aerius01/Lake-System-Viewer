...
public class MoonController : MonoBehaviour
{
    ...
    {
        CalculateNewMoonPos();
        //skyTransform.transform.localRotation = Quaternion.Euler(new Vector3(90 - currentZenithAngleDeg, currentAzimuthAngleDeg, 0f));
    }
    ...
    private void CalculateNewMoonPos()
    {
        ...
        // Final values
        double altitude = rad_to_deg(Math.Asin(Math.Sin(deg_to_rad(latitude)) * Math.Sin(deg_to_rad(declination))
            + Math.Cos(deg_to_rad(latitude)) * Math.Cos(deg_to_rad(declination)) * Math.Cos(deg_to_rad(localHourAngle))));
        
        double azimuth = localHourAngle > 180 ?
