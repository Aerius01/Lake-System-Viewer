...
public class MoonController : MonoBehaviour
{
    ...
    public void AdjustMoonPosition(Vector3 sunRotation)
    {
        // CalculateNewMoonPos(); --> was "loose" and would sometimes rise during the day, reliable moon eqs are hard to find for free, as it turns out.
        // Just have the moon be opposite the sun on the orbital plane
        this.altitude = -sunRotation.x;
        this.azimuth = sunRotation.y + 180f > 360f ? sunRotation.y - 180f : sunRotation.y + 180f;
        
        MoonPhase();
        this.moonMaterial.SetFloat("Vector1_0bdb1e1f24484093bf09898602915822", moonPhase);
        this.transform.localEulerAngles = new Vector3((float)this.altitude, (float)this.azimuth, 0f);

        // Have the moon look at the camera
        this.moonObject.transform.LookAt(Camera.main.transform.position, this.transform.up);
        Vector3 rotationEuler = moonObject.transform.localEulerAngles;
        rotationEuler.y = rotationEuler.y - 90f;
        this.moonObject.transform.localEulerAngles = rotationEuler;

        // Have the moonlight directed to the center of the mesh & manage intensity
        this.moonLight.transform.LookAt(LocalMeshData.meshCenter, this.transform.up);
        ...
        this.moonLight.GetComponent<Light>().intensity = intensity / maxVal * 0.1f;

        // Have moon fade when near & below horizon
        if (Math.Abs(this.altitude) <= 15 || Math.Abs(this.altitude) >= 165) 
        {
            double numerator = Math.Abs(this.altitude) <= 15f ? Math.Abs(this.altitude) : 180f - Math.Abs(this.altitude);
            this.moonMaterial.SetFloat("Vector1_46109bf20a484c2caf68bdeec6ce74a7", (float)numerator / 15f);
        }
        else if (this.moonMaterial.GetFloat("Vector1_46109bf20a484c2caf68bdeec6ce74a7") != 1f) this.moonMaterial.SetFloat("Vector1_46109bf20a484c2caf68bdeec6ce74a7", 1f);
    }
    ...
    private void MoonPhase()
    {
        ...
        this.moonPhase = (float)(mod(diffJulianDate, phasePeriod) / phasePeriod);
    }
...
//         double trueLambdaDeg = meanLambda;
...
//         double trueBetaDeg = meanBeta;

//         // Ecliptic calc
//         double trueEclipticOfDateDeg = eclipticOfDate + deltaE / 3600f;
//         double intermed = Math.Atan( Math.Cos(trueEclipticOfDateDeg)*Math.Cos(trueBetaDeg)*Math.Sin(trueLambdaDeg) - Math.Sin(trueEclipticOfDateDeg)*Math.Sin(trueBetaDeg)/(Math.Cos(trueBetaDeg)*Math.Cos(trueLambdaDeg)+Math.Sqrt(1-Math.Pow((Math.Sin(trueEclipticOfDateDeg)*Math.Cos(trueBetaDeg)*Math.Sin(trueLambdaDeg) + Math.Cos(trueEclipticOfDateDeg)*Math.Sin(trueBetaDeg)),2))));
//         while (intermed < 0) intermed += Math.PI;
//         double rightAscension = (180/Math.PI)*intermed; // in degrees
...
//         // Debug.Log(string.Format("long: {0:0.###}, lat: {1:0.###}; decl: {2:0.###}, RA: {3:0.###}", trueLambdaDeg, trueBetaDeg, declination, rightAscension));
...
//         Vector2 longVector = new Vector2((float)Math.Cos(deg_to_rad(this.glong)), (float)Math.Sin(deg_to_rad(this.glong)));
//         double localHourAngle = Vector2.Angle(GMSTVector, longVector);
//         // double localHourAngle = GMST + this.glong - rightAscension;
//         // while (localHourAngle < 0) localHourAngle += 360;

//         // Correction for LHA selection based on hemisphere
//         if (GMST < this.glong + 180 && GMST > this.glong) localHourAngle = 360 - localHourAngle;

//         // Final values
//         // altitude = rad_to_deg(Math.Asin(Math.Sin(deg_to_rad(this.glat)) * Math.Sin(deg_to_rad(declination))
//         //     + Math.Cos(deg_to_rad(this.glat)) * Math.Cos(deg_to_rad(declination)) * Math.Cos(deg_to_rad(localHourAngle))));
        
//         // azimuth = localHourAngle > 180 ?
//         //     rad_to_deg(Math.Acos((Math.Sin(deg_to_rad(declination)) - Math.Sin(deg_to_rad(altitude))
//         //     * Math.Sin(deg_to_rad(this.glat))) / (Math.Cos(deg_to_rad(altitude)) * Math.Cos(deg_to_rad(this.glat))))) :
//         //     360 - (rad_to_deg(Math.Acos((Math.Sin(deg_to_rad(declination)) - Math.Sin(deg_to_rad(altitude))
//         //     * Math.Sin(deg_to_rad(this.glat))) / (Math.Cos(deg_to_rad(altitude)) * Math.Cos(deg_to_rad(this.glat))))))
//         // ;

//         altitude = Math.Asin(Math.Sin(this.glat * Math.PI/180f)*Math.Sin(declination* Math.PI/180f) + Math.Cos(this.glat* Math.PI/180f)*Math.Cos(declination* Math.PI/180f)*Math.Cos(localHourAngle* Math.PI/180f)) * 180/Math.PI;
//         // azimuth = localHourAngle > 180 ?
//         //     Math.Atan2(Math.Sin(localHourAngle* Math.PI/180f), (Math.Cos(this.glat* Math.PI/180f)*Math.Tan(declination* Math.PI/180f) - Math.Sin(this.glat* Math.PI/180f)*Math.Cos(localHourAngle* Math.PI/180f))) * 180/Math.PI : 
//         //     360 - Math.Atan2(Math.Sin(localHourAngle* Math.PI/180f), (Math.Cos(this.glat* Math.PI/180f)*Math.Tan(declination* Math.PI/180f) - Math.Sin(this.glat* Math.PI/180f)*Math.Cos(localHourAngle* Math.PI/180f))) * 180/Math.PI;
//         azimuth = Math.Atan2(Math.Sin(localHourAngle* Math.PI/180f), (Math.Cos(this.glat* Math.PI/180f)*Math.Tan(declination* Math.PI/180f) - Math.Sin(this.glat* Math.PI/180f)*Math.Cos(localHourAngle* Math.PI/180f))) * 180/Math.PI;

//         Debug.Log(string.Format("azimuth: {0:0.00}; altitude: {1:0.00}", azimuth, altitude));
//         // Debug.Log(string.Format("{0} == {1}; {2} == {3}", altitude, h * 180f/Math.PI, azimuth, az* 180f/Math.PI));
//     }
