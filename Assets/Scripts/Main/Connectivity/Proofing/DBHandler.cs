...
public class DBHandler : MonoBehaviour
{
    ...
    private TableProofings tableProofings;
    ...
    public async void VerifyButton()
    {
        ...
        await using (NpgsqlConnection connection = new NpgsqlConnection(connString))
        {
            ...
            try
            {
                tableProofings = new TableProofings(connection, loadingBar);
                bool requiredTables = await tableProofings.VerifyTables();

                if (TableProofings.tables.Keys.Count == 0) throw new NpgsqlException();
                ...
                foreach (string key in TableProofings.tables.Keys)
                {
                    ...
                    Table table = TableProofings.tables[key];
    ...
    public async void StartButton()
    {
        ...
        else 
        {
            ...
            foreach (Table table in TableProofings.tables.Values) { if (!table.imported) failedTables.Add(table.tableName); }
