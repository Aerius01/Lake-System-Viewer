...
public class TableProofings
{
    ...
    public TableProofings(NpgsqlConnection connection, LoaderBar loadingBar)
    {
        this.connection = connection;
        this.loadingBar = loadingBar;
        TableProofings.tables = new Dictionary<string, Table>(); 

        // The ordering of these names is important
        TableProofings.checkTables = new List<string>() { "fish", "meshmap", "positions_local", "macromap_polygons_local", "macromap_heights_local", "species", "weatherstation", "thermocline" };
    }
    ...
    private async Task<bool> Verificator()
    {
        ...
        this.loadingBar.SetText(TableProofings.checkTables[0]);
        this.fishAccepted = await Task.Run(() => this.CheckFishTable(TableProofings.checkTables[0]));

        this.loadingBar.SetText(TableProofings.checkTables[1]);
        Tuple<bool, int, int, float, float> heightMapPacket = await Task.Run(() => this.CheckHeightMap(TableProofings.checkTables[1]));
        ...
        this.loadingBar.SetText(TableProofings.checkTables[2]);
        this.positionsAccepted = await Task.Run(() => this.CheckPositionsTable(TableProofings.checkTables[2], heightMapPacket.Item2, heightMapPacket.Item3, heightMapPacket.Item4, heightMapPacket.Item5));

        this.loadingBar.SetText(TableProofings.checkTables[3]);
        this.macroPolysAccepted = await Task.Run(() => this.CheckMacroPolyTable(TableProofings.checkTables[3], heightMapPacket.Item2, heightMapPacket.Item3));

        this.loadingBar.SetText(TableProofings.checkTables[4]);
        this.macroHeightsAccepted = await Task.Run(() => this.CheckMacroHeightTable(TableProofings.checkTables[4], heightMapPacket.Item2, heightMapPacket.Item3));
    

        this.loadingBar.SetText(TableProofings.checkTables[5]);
        this.speciesAccepted = await Task.Run(() => this.CheckSpeciesTable(TableProofings.checkTables[5]));

        this.loadingBar.SetText(TableProofings.checkTables[6]);
        this.weatherAccepted = await Task.Run(() => this.CheckWeatherTable(TableProofings.checkTables[6]));

        this.loadingBar.SetText(TableProofings.checkTables[7]);
        this.thermoclineAccepted = await Task.Run(() => this.CheckThermoTable(TableProofings.checkTables[7]));
    ...
    private async Task<bool> CheckFishTable(string tableName)
    {
        ...
        lock(operationLocker) TableProofings.tables[tableName] = fishTable;
    ...
    private async Task<Tuple<bool, int, int, float, float>> CheckHeightMap(string tableName)
    {
        ...
        TableProofings.tables[tableName] = heightMapTable;
    ...
    private async Task<bool> CheckPositionsTable(string tableName, int rowCount, int columnCount, float minDepth, float maxDepth)
    {
        ...
        TableProofings.tables[tableName] = positionsTable;
    ...
    private async Task<bool> CheckThermoTable(string tableName)
    {
        ...
        TableProofings.tables[tableName] = thermoTable;
    ...
    private async Task<bool> CheckSpeciesTable(string tableName)
    {
        ...
        TableProofings.tables[tableName] = speciesTable;
    ...
    private async Task<bool> CheckWeatherTable(string tableName)
    {
        ...
        TableProofings.tables[tableName] = weatherTable;
    ...
    private async Task<bool> CheckMacroPolyTable(string tableName, int rowCount, int columnCount)
    {
        ...
        TableProofings.tables[tableName] = polyTable;
        ...
        if (!this.heightMapAccepted)
        {
            ...
            polyTable.SetMessage(string.Format("RED: The successful import of this table is dependent on that of the \"{0}\" table.", TableProofings.checkTables[1])); 
            conditionsMet = false; 
        }
        // PRIMARY: The table must be present
        else if (this.tableNames.Contains(tableName))
    ...
    private async Task<bool> CheckMacroHeightTable(string tableName, int rowCount, int columnCount)
    {
        ...
        TableProofings.tables[tableName] = heightTable;
        ...
        if (!this.macroPolysAccepted)
        {
            ...
            heightTable.SetMessage(string.Format("RED: The successful import of this table is dependent on that of the \"{0}\" table.", TableProofings.checkTables[3])); 
            conditionsMet = false; 
        }
        // PRIMARY: The table must be present
        else if (this.tableNames.Contains(tableName))
