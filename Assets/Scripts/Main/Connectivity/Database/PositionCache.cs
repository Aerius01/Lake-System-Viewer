...
public class PositionCache
{
    ...
    {
        // Each fish calls for the creation of its own specific cache
        this.fishID = fishID;
    ...
    public DataPacket[] GetCachedBounds(DateTime timestamp)
    {
        lock(this.locker)
        {
            ...
            {
                // The next data point exists within the cache, in the future cached data packets
            ...
            {
                // The next data point exists within the cache, in the past cached data packets
                // If the timestamp is exactly equal to the last timestamp in backwardCache, then no changes to the lists are necessary
    ...
    {
        // When new positions come in from DatabaseConnection, add them to the fish's forward/backward caches
        if (forwardOnly)
        {   
            // We only asked for future positions
            lock(this.locker) 
            { 
        ...
        {
            // We asked for both future and past positions. Find where the present is to split the list into past and future positions.
            ...
            {    
                ...
                // Allocate packets based on the split. If SplitIndex < 0, we're in between two timestamps
                if (splitIndex < 0)           
                {
