...
public class DatabaseConnection
{
    ...
    private NpgsqlBatchCommand NewMetadataBatchCommand(int key, List<string> presentFishColumns)
    {
        ...
        if (TableProofings.tables[TableProofings.checkTables[5]].imported && presentFishColumns.Contains("f.species"))
        {
            sql = string.Format(
    ...
    public async Task<Dictionary<int, FishPacket>> GetFishMetadata(List<int> keyList)
    {
        ...
        await using (NpgsqlConnection connection = new NpgsqlConnection(connString))
        {
            if (await DatabaseConnection.Connect(connection))
            {
                ...
                foreach (string column in optionalFishColumns) { if (TableProofings.tables[TableProofings.checkTables[0]].presentColumns.Contains(column)) { presentFishColumns.Add("f." + column); } }
    ...
    public static async Task<WeatherPacket> GetWeatherData()
    {
        ...
        foreach (string column in optionalWeatherColumns) { if (TableProofings.tables[TableProofings.checkTables[6]].presentColumns.Contains(column)) { presentWeatherColumns.Add(column); } }
    ...
    public async static Task<ThermoPacket> GetThermoData()
    {
        ...
        if (TableProofings.tables[TableProofings.checkTables[7]].presentColumns.Contains("oxygen"))
        {
            sql = string.Format(
            @"SELECT timestamp,
                depth,
