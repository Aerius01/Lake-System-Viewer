...
public class GradientPicker : MonoBehaviour
{
    ...
    private void CreateColorKey(GradientColorKey k)
    {
        if (colorKeys.Count < 8)
        {
            ...
            s.name = string.Format("ColorKey{0}", colorKeys.Count);
    ...
    private void CreateAlphaKey(GradientAlphaKey k)
    {
        if (alphaKeys.Count < 8)
        {
            ...
            s.name = string.Format("AlphaKey{0}", alphaKeys.Count);
            s.gameObject.SetActive(true);
            s.value = k.time;
    ...
    public void CheckDeleteKey(Slider s)
    {
        ...
        {
            if (s.name.Contains("ColorKey") && colorKeys.Count > 2)
            {
            ...
            if(s.name.Contains("AlphaKey") && alphaKeys.Count > 2)
            {
    ...
    {
        ColorPickerImported.Create(colorKeys[selectedColorKey].color, "Gradient Color Key", (c) => UpdateColor(selectedColorKey, c), null);
    }
